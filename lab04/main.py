...
class MainWindow:
    ...
    def create_interface(self):
        ...
        self.status_text.tag_configure("fcs", foreground="orange")
    ...
    def make_fcs_status(self, message):
        self.status_text.config(state='normal')
        self.status_text.insert(tk.END, message, "fcs")
        self.status_text.see("end")
        self.status_text.config(state='disabled')
    ...
    def make_stuff_package_status(self, stuff_package: str, collision_count: int):
        ...
        for i in range(len(str_stuff_package)):
            if i >= index_fcs and is_first_fcs:
                self.make_status(' ', "")
                is_first_fcs = False
    ...
    def make_stuff_package_output(self, stuff_package: str):
        if stuff_package != "":
            is_mistake, payload = self.crc.reciverSide(stuff_package)
